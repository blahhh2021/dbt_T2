...
def retract(ip):
    set_digital_io(2,True, ip)

def turn_extrusion_on(speed, ip):
    set_digital_io(1,False,ip)
    if speed == 0:
        set_digital_io(2,False, ip)
        set_digital_io(3,True, ip)
    elif speed == 1:
        set_digital_io(2,True, ip)
        set_digital_io(3,False, ip)

def send_printpath(frames, velocities, accelerations, radii, toggles, retractSwitch = False, ip = '127.0.0.1'):
    ...
    try:
        while ur_c.getAsyncOperationProgress() >= 0:
            ...
            if new_waypoint != waypoint:
                ...
                # Using Retraction
                if  change_toggle and retractSwitch:
                    if toggles[counter] == 1: 
                        turn_extrusion_on(speed = 0,ip= ip)
                    elif toggles[counter] == 2:
                        retract(ip)
                    else:
                        set_digital_io(1, True, ip)
                    cur_time = time.time()
